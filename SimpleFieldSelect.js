define(["qlik", "jquery", "css!./SimpleFieldStyle.css", "text!./datepicker.css", "./properties.def", "text!./select2/select2.css", "./select2/select2.min"], function (qlik, $, cssContent, cssDatepick, propertiesdef, select2css) {
  'use strict';
  var debug = 0;
  ...
  function checkDefaultValueSelection($element, countselected, layout, self, app, sfssettings) {
    if (debug)
      console.log('checkin default selection, selected: ' + countselected);
    ...
    if (selectDefaults && layout.props.selectDefaultsOnlyOnce && sfsdefaultselstatus[layout.qInfo.qId] == 1) {
      if (debug)
        console.log('Defaults already selected');
      return;
    }
    if (countselected == 0 && selectDefaults) {
      ...
      if (!layout.props.selectOnlyOne && otherdefaultelememnts.length > 0) {
        if (debug)
          console.log('selecting other defaults, method: ' + otherDefaultElementsSelectionStyle);
  ...
  function createglobalCSS(pr) {
    ...
    if (pr.global_customselector) {
      ...
      selectors.forEach(function (i) {
        ...
        cObjSelectors.push('.qv-object-' + i + ' .qv-inner-object');
      });
    ...
    if (pr.global_bordercolor) {
      gcss += ' .sheet-grid .qv-gridcell:not(.qv-gridcell-empty) { border-color:' + pr.global_bordercolor + ';}';
    }
    if (typeof pr.global_bordercolor2 === 'undefined') {
      gcss += ' .sheet-grid .qv-gridcell:not(.qv-gridcell-empty) { border-color:' + pr.global_bordercolor + ';}';
    } else if (pr.global_bordercolor2) {
    ...
    if (pr.global_elementbgcolor && pr.global_elementbgcolor != '') {
      if (customObjectSelector) {
        ...
        gcss += ' .qv-client #qv-stage-container #grid .qv-object-wrapper .qv-inner-object, .qv-client.qv-card #qv-stage-container #grid .qv-object-wrapper .qv-inner-object {background-color:' + pr.global_elementbgcolor + '!important;}';
      }
    }
    ...
    if (pr.hideGuiToolbar && $(".qv-mode-edit").length == 0) {
      if ($("#qv-toolbar-container,.qv-toolbar-container").length > 0) {
    ...
    if (pr.toolbarTxt && !pr.hideGuiToolbar) {
      if ($("#sfstoolbartxt").length == 0) {
        if ($(".qui-buttonset-left").length > 0) {
    ...
    if (pr.globsfs_stateS_bg) {
      gcss += objsel + 'li.stateS {background-color:' + pr.globsfs_stateS_bg + ';}';
    }
    if (pr.globsfs_stateO_bg) {
      gcss += objsel + 'li.stateO {background-color:' + pr.globsfs_stateO_bg + ';}';
    }
    if (pr.globsfs_stateA_bg) {
      gcss += objsel + 'li.stateA {background-color:' + pr.globsfs_stateA_bg + ';}';
    }
    if (pr.globsfs_stateX_bg) {
      gcss += objsel + 'li.stateX {background-color:' + pr.globsfs_stateX_bg + ';}';
    }
    if (pr.globsfs_stateS_fo) {
      gcss += objsel + 'li.stateS {color:' + pr.globsfs_stateS_fo + ';}';
    }
    if (pr.globsfs_stateO_fo) {
      gcss += objsel + 'li.stateO {color:' + pr.globsfs_stateO_fo + ';}';
    }
    if (pr.globsfs_stateA_fo) {
      gcss += objsel + 'li.stateA {color:' + pr.globsfs_stateA_fo + ';}';
    }
    if (pr.globsfs_stateX_fo) {
      gcss += objsel + 'li.stateX {color:' + pr.globsfs_stateX_fo + ';}';
    }
    if (pr.globsfs_border) {
      gcss += objsel + '.sfe,.qv-object-SimpleFieldSelect ul li {border-color:' + pr.globsfs_border + ';}';
    }
    if (pr.globsfs_hoverBG) {
      gcss += objsel + '.sfe:hover {background-color:' + pr.globsfs_hoverBG + '!important; transition: 0.2s;}';
    }
    if (pr.globsfs_hoverFontcolor) {
      gcss += objsel + '.sfe:hover {color:' + pr.globsfs_hoverFontcolor + '!important; transition: 0.2s;}';
    }
    objsel = ' .qv-object-SimpleFieldSelect .sfsdiv ';
    if (pr.globsfs_leftpad) {
      gcss += objsel + '{padding-left:' + pr.globsfs_leftpad + ';}';
    }
    if (pr.globsfs_bottompad) {
      gcss += objsel + '{padding-bottom:' + pr.globsfs_bottompad + ';}';
    }
    if (pr.globsfs_rightpad) {
      gcss += objsel + '{padding-right:' + pr.globsfs_rightpad + ';}';
    }
    if (pr.globsfs_toppad) {
      gcss += objsel + '{padding-top:' + pr.globsfs_toppad + ';}';
    }
    if (pr.globsfs_custompad) {
      gcss += objsel + '{padding:' + pr.globsfs_custompad + ';}';
    }
  ...
  function selectValueInQlik(self, value, layout, app, selectvalueMethod, $element, sfssettings) {
    if (layout.props.dimensionIsVariable) {
      if (debug)
        console.log('set variable value to ' + value);
      if (!(sfssettings.variableOptionsForValuesArray && sfssettings.variableOptionsForValuesArray.length > 0)) {
        if (debug)
          console.log('No values in variableOptionsForValuesArray');
        return;
      }
      if (layout.props.variableName == '' || !layout.props.variableName || typeof layout.props.variableName == 'object') {
        if (debug)
          console.log('Variable name is empty');
        return;
      }
      var valueTxt = '';
      ...
      if (layout.props.varMultiselectAllow) {
        ...
        if (varvaluelist) {
          ...
          if (debug) {
            console.log('multivalues to select to var ');
        ...
        if (debug)
          console.log('Picked variable ' + value + ' ' + valueTxt);
      ...
      if (debug)
        console.log(' means ' + valueTxt + ' to variable ' + layout.props.variableName);
      ...
      if (sfssettings.variableOptionsForKeysArray != [] && layout.props.variableNameForKey && sfssettings.variableOptionsForKeysArray[value]) {
        ...
        if (debug)
          console.log(' key value ' + keyTxt + ' to variable ' + layout.props.variableNameForKey);
      ...
      if (layout.props.disableToggleMode) {
        selectvalueMethod = false;
      }
      if (debug)
        console.log('set value to index ' + value + ' method ' + selectvalueMethod);
      if (isNaN(value)) {
  ...
  function selectValuesInQlik(self, values, layout, app, selectvalueMethod, $element, sfssettings) {
    if (debug) {
      console.log('set values to indexes ');
      console.log(values);
    }
    if (layout.props.dimensionIsVariable) {
  ...
  function setSmallDeviceStuff($element, pr) {
    ...
    if (pr.enableGlobals && pr.gremoveMobileZoom) {
      $(".transparent-overlay").remove();
    } else
      if (pr.mobileRemoveZoom) {
        parent.find('.transparent-overlay').remove();
      }
    if (!pr.removeYscroll) {
      parent.find('.qv-object-content-container').addClass('enableyscroll');
    }
  ...
  return {
    ...
    controller: ['$scope', '$element', function ($scope, $element) {
        ...
        if (scpr.enableGlobals && scpr.global_bghtml) {
          $scope.$on('$destroy', function (ev) {
            $("#sfsgridbg").remove();
          });
        }
        $scope.$on('$destroy', function (ev) {
          var contextmenuID = 'sfsrmenu';
          $("." + contextmenuID + ",.sfssel2c,#sfsselect2").remove();
        });
      }
    ],
    paint: function ($element, layout) {
      if (debug) {
        console.log('start painting ' + layout.qInfo.qId);
        console.log($element);
      }
      var self = this,
      html = "";
      ...
      if (layout.qListObject.qDataPages[0] && layout.qListObject.qDataPages[0].qMatrix.length > 0) {
        ...
        if (allrowscount > rowcount) {
          var lastrow = 0;
          this.backendApi.eachDataRow(function (rownum, row) {
            lastrow = rownum;
          });
          if (debug)
            console.log('fetching more rows ' + lastrow);
      ...
      if (pr.contentpadding && pr.contentpadding != '-') {
        containerDivHeight_reduce += (parseInt(pr.contentpadding) * 2);
      ...
      if (pr.hideFieldsFromSelectionBar || pr.hideFromSelectionsBar) {
        if ($(".sfshideselstyles").length > 0) {}
          ...
          $('.qvt-selections,.qs-header').append('<div style="display:none;" class="sfshideselstyles"></div>');
        }
        ...
        if (pr.hideFromSelectionsBar) {
          ...
          if (fieldToHide == '' || !fieldToHide) {
            fieldToHide = layout.qListObject.qDimensionInfo.qGroupFieldDefs[0];
            fieldToHide = fieldToHide.replace(/[\[\]']+/g, '');
            var fieldToHide2 = fieldToHide;
            if (fieldToHide.slice(0, 1) === '=') {
      ...
      if (pr.enableGlobals) {
        if (debug)
          console.log('enabled globals ' + layout.qInfo.qId);
      ...
      if (pr.dimensionIsVariable) {
        ...
        if (debug) {
          console.log('varvalue from ' + pr.variableName + ' is ');
      ...
      if (pr.variableIsDate && pr.dimensionIsVariable) {
        if ($("#sfsdatepicker").length > 0) {}
          ...
          $("<style id=sfsdatepicker>").html(cssDatepick).appendTo("head");
        }
        if (pr.variableName) {
          if (debug) {
            console.log('varvalue ' + pr.variableName);
            console.log(app.variable.getContent(pr.variableName));
          }
          ...
          if (pr.visInputPlaceholdertxt && pr.visInputPlaceholdertxt != '') {
            html += ' placeholder="' + pr.visInputPlaceholdertxt.replace(/\"/g, '&quot;') + '"';
          }
          ...
          if (pr.maxLimitvariable && pr.maxLimitvariable && pr.maxLimitvariable != '-') {
            if (debug) {
              console.log('Limiting days to ');
              console.log(pr.maxLimitvariable);
            }
          ...
          $element.find('.pickdate').on('change', function () {
            var newval = $(this).val();
            if (debug)
              console.log('NEW ' + newval + ' to ' + pr.variableName);
        ...
        $element.html(html);
        $element.find('.htmlin').on('change select', function () {
          var newval = $(this).val();
        ...
        if (pr.visInputFieldType == 'range') {
          ...
          targetelement[0].oninput = function (e) {
        ...
        if (pr.textareaonlytext)
          html += sfsSanitize(pr.textareaonlytext);
        if (pr.textareaonlytext2)
          html += sfsSanitize(pr.textareaonlytext2);
        if (pr.helptext) {
          html += createhelptextdiv(pr);
        }
        html += '</div>';
        ...
        if (visType == 'vlist') {
          ...
          html += '<select class="dropdownsel' + elementExtraClass + createLUIclass(pr.addLUIclasses, visType, '') + '" style="' + fontsizechanges + fontStyleTxt + elementStyleCSS + bordercolorstyle + containerStyles + mainobjectstyle + '"' + elementExtraAttribute + multiselect + '>';
        ...
        optionsPrintGroupHeaderBeforeValue = {};
        if (pr.dimensionIsVariable) {
          if (debug)
            console.log('variable value ' + varvalue);
          if (debug && pr.varMultiselectAllow) {
            console.log('paint multivalues');
          ...
          splitted.forEach(function (opt) {
            ...
            sfssettings.variableOptionsForValuesArray.push(opt);
            if (pr.varRemovebrackets) {
              opt = opt.replace(/"|\]|\[/g, '');
            }
            optionsforselect.push([{
                  qState: qState,
                  qText: opt,
                  qElemNumber: varindex
                }
              ]);
            varindex += 1;
          });
          if (debug) {
            console.log(pr.variableOptionsForValues);
          }
          var varKeyindex = 0;
          if (pr.variableOptionsForKeys != '') {
            splitted = sfsSanitize(pr.variableOptionsForKeys).split(";");
            splitted.forEach(function (opt) {
              sfssettings.variableOptionsForKeysArray.push(opt);
              varKeyindex += 1;
            });
        ...
        if (!pr.selectOnlyOne && pr.selectAlsoThese && pr.selectAlsoThese != '' && !pr.dimensionIsVariable && visType != 'dropdown' && visType != 'btn' && visType != 'radio') {
        ...
        var calcSelected = 0,
        calcTotalRows = optionsforselect.length;
        ...
        if (visType == 'searchonly') {
          ...
          optionsforselect.forEach(function (row) {
            if (pr.hidePassiveItems && !pr.dimensionIsVariable && row[0].qState === 'X') {
              return;
            }
            ...
            var defaultelementclass = '',
            checkedstatus = '',
            dis = '',
            selectedClass = '',
            dropselection = '',
            otherdefaultelementclass = '';
            if (row[0].qState === 'S') {
            ...
            if (pr.selectOnlyOne && !pr.dimensionIsVariable && countselected > 1) {
              if (debug)
                console.log('Select only one enabled, reducing selections.');
              checkedstatus = '';
              selectedClass = '';
              dropselection = '';
              if (visTypedropdownOrSelect2) {
        ...
        if (showContextMenu) {
          ...
          if (debug)
            console.log('create context menu ' + contextmenuID)
          ...
          if (pr.rightclikcmenu_selall)
            contextmenuHtml += '<li act="all">Select all</li>';
          if (pr.rightclikcmenu_clear)
            contextmenuHtml += '<li act="clear">Clear selections</li>';
          if (pr.rightclikcmenu_reverse)
            contextmenuHtml += '<li act="reverse">Reverse selection</li>';
          if (pr.rightclikcmenu_possible)
            contextmenuHtml += '<li act="possible">Select possible</li>';
          if (pr.rightclikcmenu_random)
            contextmenuHtml += '<li act="random">Select randomnly</li>';
          if (pr.rightclikcmenu_defaults)
            contextmenuHtml += '<li act="defaults">Select defaults</li>';
          if (pr.rightclikcmenu_copy)
            contextmenuHtml += '<li act="copy">Copy values to clipboard</li>';
          contextmenuHtml += '</ul></div>';
          $('body').append(contextmenuHtml);
          sfsrmenu = $("." + contextmenuID);
          if (debug)
            console.log('set contextmenu action on ' + layout.qInfo.qId);
          if (pr.rigthclickmenushowasicon) {
            ...
            if (debug)
              console.log('Add cntx menulaunch' + $element.find(".contextmenulaunch").length);
          ...
          function showcontextmenuevent(event) {
            if (debug)
              console.log('show contextmenu ' + contextmenuID);
            event.preventDefault();
            hidermenu(event);
            sfsrmenu.finish().toggle(100).css({
              top: (event.pageY + 45) + "px",
              left: (event.pageX - 30) + "px"
            });
            sfsrmenu = $("." + contextmenuID);
            if (debug)
              console.log(sfsrmenu.find('li'));
            sfsrmenu.find('li').click(function () {
              ...
              if (action == 'getselectionurl') {
                ...
                if (dataelements.length > 0) {}
                  ...
                  dataelements = $element.find('.data');
                }
          ...
          function hidermenu(e) {
            if (!$(e.target).parents("." + contextmenuID).length > 0) {
              if (debug)
                console.log('menuhide ' + contextmenuID);
              $(".sfsrmenu").hide(100);
            }
            $(document).off('mousedown', hidermenu);
          }
        ...
        if (visType == 'hlist' || visType == 'vlist') {
          ...
          if (!pr.dimensionIsVariable) {
            ...
            articleElement.on('mouseup mouseleave touchend', function () {
              isMouseDown = false;
              if (debug)
            ...
            listtargets.on('mouseenter touchmove', function () {
              if (isMouseDown) {
                if (!$(this).hasClass('selected')) {
                  if (debug)
                    console.log('adding selectclass');
          ...
          listtargets.on('qv-activate', function (ev) {
            var clicktarget = $(this);
            var targetValueID = parseInt(clicktarget.attr("dval"), 10);
            if (debug)
              console.log('qv active on list change ' + targetValueID);
            if (!$(this).hasClass('selected')) {
              if (debug)
              ...
              if (pr.dimensionIsVariable && pr.variableEmptyAlreadySelected) {
          ...
          if (visTypedropdownOrSelect2) {
            ...
            dropdownelem.on('change', function () {
              if (debug)
                console.log('select change action');
              if (!layout.props.selectmultiselect) {
            ...
            if (visType == 'checkbox' || visType == 'luiswitch' || visType == 'luicheckbox') {
              $element.find('input').on('click', function () {
                if (debug)
                  console.log('checkbox clicked action');
                var clicktarget = $(this);
                if ($(this).attr("dval")) {
                  var targetValueID = parseInt(clicktarget.attr("dval"), 10);
                  if ($(this).is(':checked')) {
                    ...
                    if (debug)
                      console.log(clicktarget.attr("dval"));
                    selectValueInQlik(self, targetValueID, layout, app, true, $element, sfssettings);
                    var selectedCount = 0;
                    $element.find('input:checked').each(function () {
                      selectedCount += 1;
              ...
              if (visType == 'btn') {
                ...
                $element.find('input').on('click', function () {
                  if (debug)
                    console.log('radio change action');
                  var clicktarget = $(this);
                  var targetValueID = parseInt(clicktarget.attr("dval"), 10);
                  $element.find('.selected').each(function () {
                    var value = parseInt($(this).attr("dval"), 10);
      ...
      if (visType == 'select2') {
        if (debug)
          ...
          if (!$("#sfsselect2").length > 0) {
            $("<style id=sfsselect2>").html(select2css).appendTo("head");
          }
          ...
          'templateSelection': function (data, container) {
            if (data.element) {
              ...
              if ($(data.element).hasClass("disableBGimage")) {
                $(container).addClass('disableBGimage');
              }
            }
            return data.text;
          }
        ...
        select2container.attr('style', select2container.attr('style') + ' ' + selectElement.attr('style'));
      }
      if (pr.enablesearch) {
        ...
        searchField.keyup(function (e) {
          if (e.which == 27) {
            $element.find('#cl' + searchId).trigger('click');
          }
        });
      ...
      function searchOnlySearch(searchDictionary, filters, filtercount, searchmethod) {
        var toSelectWithSearchOnly = [];
        if (searchmethod && searchmethod > 0) {
      ...
      function createhelptextdiv(pr) {
        ...
        if (pr.helptextcss)
          a += ' style="' + checkUserCSSstyle2(pr.helptextcss, 1) + '"';
